...
public class AllianceOrderFragment extends BaseFragment {
    @BindView(R.id.tl_tab)
    TabLayout mTabLayout;
    @BindView(R.id.vp_pager)
    ViewPager mViewPager;
    String[] mTitle = {"未开始", "进行中", "已完成"};
    OrderBeforeAllianceFragment orderBeforeFragment = new OrderBeforeAllianceFragment();
        ...
        mViewPager.setAdapter(new FragmentPagerAdapter(getChildFragmentManager()) {
            ...
            public Fragment getItem(int position) {
                ...
                if (position == 0)
                    ...
                    fragment = new OrderIngAllianceFragment();
                else if (position == 2)
                    fragment = new OrderAfterAllianceFragment();
                return fragment;
            }
